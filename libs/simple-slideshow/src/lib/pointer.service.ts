...
export class PointerService {
  ...
  private _convertBGPosToPixels(e: PointerEvent): void {
    if (this._originalState.valid) {
      ...
      if (bgPosX.indexOf('px') === -1) {
        ...
        if (bgPosX.indexOf('%') > -1) {
          const bgPosXPercentage = Number(bgPosX.substring(0, bgPosX.length - 1)) / 100
      ...
      if (bgPosY.indexOf('px') === -1) {
        ...
        if (bgPosY.indexOf('%') > -1) {
          const bgPosYPercentage = Number(bgPosY.substring(0, bgPosY.length - 1)) / 100
  ...
  private _currentBGSize(e: PointerEvent): number {
    ...
    if (bgSize.indexOf(' ') > -1) {
      // backgroundSize pattern "auto 100px" or "100px auto" or "100px 200px"
      const sizeTuple = bgSize.split(' ')
      const size = this._originalState.widthBound
        ? sizeTuple[0].substring(0, sizeTuple[0].length - 2)
        : sizeTuple[1].substring(0, sizeTuple[1].length - 2)
      return Number(size)
    } else if (bgSize.indexOf('px') > -1) {

...
export class TimeEffects {
  ...
  loadTimes$ = createEffect(() => this.actions$.pipe(
    ...
    concatMap(() => this.afs.collection<Time>('info').doc<Time>('time').valueChanges().pipe(
      map(e => setTimes(e)),
      catchError(e => {console.error(e); return EMPTY}) // TODO(zoltanszepesi): proper error handlings
    ))
  ));

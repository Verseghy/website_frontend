...
export function problemReducer(state: State = initialState, action: actions.ProblemActions) {
  switch (action.type) {
    case actions.ADDED:
...
export const { selectIds, selectEntities, selectAll, selectTotal } = problemsAdapter.getSelectors(getProblemState)

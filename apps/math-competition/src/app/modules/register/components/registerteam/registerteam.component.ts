...
export class RegisterteamComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder, private afStore: AngularFirestore, private afAuth: AngularFireAuth, private route: Router) {}
  ...
  onChange() {
    ...
    if (
      (<FormGroup>(<FormArray>this.teamForm.controls.members).controls[(<FormArray>this.teamForm.controls.members).length - 1]).controls
        .email.value !== null &&
      (<FormGroup>(<FormArray>this.teamForm.controls.members).controls[(<FormArray>this.teamForm.controls.members).length - 1]).controls
        .name.value !== null
    ) {
